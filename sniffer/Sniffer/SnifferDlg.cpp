...
BOOL CSnifferDlg::OnInitDialog()//是list control控件要显示的内容进行初始化
{
	...
	//列表视图初始化
	m_list1.SetExtendedStyle(m_list1.GetExtendedStyle() | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);// 为列表视图控件添加全行选中和栅格风格，添加行列
	//m_list1是添加的函数变量名，insertcolumn是在对话框中插入行的函数；LVCFMT_CCENTER是为添加的行属性设置函数（居左，居中，居右，50是行列的默认值）
	//依次添加要在list1 control空格中要显示的内容
...
void CSnifferDlg::OnStart()
{
	...
	m_bFlag = true;
	CreateThread(NULL,NULL,CapturePacket,(LPVOID)this,true,NULL);//创建新的线程
}
...
//抓包函数，创建线程
DWORD WINAPI CapturePacket(LPVOID lpParam)//捕获数据包
{
	...
	//添加tree control变量函数
{
	...
	hSubItem = m_tree1.InsertItem(_T("数据链路层"),hRoot);//添加“父”类树 数据链路层
	ethernet_header *eh;
	eh = (ethernet_header *)pkt_data;//添加“子”类树
	...
	case IP:
		{
			...
			hSubItem = m_tree1.InsertItem(_T("网络层"),hRoot);//添加“父”类树 网络层
			ip_header *ih;
			const u_char *ip_data;
			ip_data=pkt_data+14;
			ih = (ip_header *)ip_data;
			...
			case TCP:
				{
					...
					hSubItem = m_tree1.InsertItem(_T("传输层"),hRoot);//添加“父”类树 传输层
					tcp_header *th;
					const u_char *tcp_data;
					tcp_data = ip_data+ip_len;
					th = (tcp_header *)tcp_data;
